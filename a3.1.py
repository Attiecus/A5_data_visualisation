...
def fetch_stock_data(ticker_symbol, period_str):
    try:
...
fast = 2.0 * np.exp(-df['t'].values / (N/6))
slow = 1.4 * np.exp(-df['t'].values / (N/2))
...
for k in range(N):
    if k == 0:
        ...
        valid_ret = df['ret'].dropna()
        if len(valid_ret) > k:
            corr = np.corrcoef(valid_ret[:-k], valid_ret[k:])[0,1] if k > 0 else 1
...
with col2:
    st.markdown(f"""
    <div class='metric-box'>
        <div class='metric-title'>VOLATILITY
                </div>
...
fig = make_subplots(
    rows=3, cols=2,
    specs=[
        [{"type": "candlestick", "rowspan": 2}, {"type": "xy"}],
        [None, {"type": "xy"}],
        [{"type": "xy"}, {"type": "xy"}]
    ],
    subplot_titles=[
...
fig.add_trace(go.Scatter(
    x=df["t"][:1], y=df["intensity"][:1],
    mode="lines", name="Total Intensity",
    line=dict(color="#00eaff", width=3)
), row=1, col=2)
fig.add_trace(go.Scatter(
    x=df["t"][:1], y=df["fast"][:1],
    mode="lines", name="Fast Decay",
    line=dict(color="#00ffaa", width=2, dash="dot")
), row=1, col=2)
fig.add_trace(go.Scatter(
    x=df["t"][:1], y=df["slow"][:1],
    mode="lines", name="Slow Decay",
    line=dict(color="#ff4d4d", width=2, dash="dot")
), row=1, col=2)
fig.add_trace(go.Scatter(
    x=df["t"][:1], y=df["baseline"][:1],
...
fig.add_trace(go.Scatter(
    x=df["t"][:1], y=df["ofi"][:1],
    mode="lines+markers", name="OFI",
    line=dict(color="#ff44cc", width=2),
...
if len(ret_clean) > 1:
    ...
    fig.add_trace(go.Scatter(
        ...
        fill='tozeroy',
        fillcolor='rgba(178, 102, 255, 0.3)'
    ), row=3, col=2)
frames = []
for i in range(2, N, max(1, N//100)):
    ret_clean = df["ret"][:i].dropna()
    if len(ret_clean) > 1:
        kde = gaussian_kde(ret_clean)
        xgrid = np.linspace(ret_clean.min(), ret_clean.max(), 200)
        kde_y = kde(xgrid)
    ...
    frames.append(go.Frame(
        data=[
            ...
            go.Scatter(x=df["t"][:i], y=df["ofi"][:i],
                      mode="lines+markers",
                      marker=dict(size=7, color=colors_frame)),
